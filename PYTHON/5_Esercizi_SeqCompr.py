...
def expBase():
    return [(exp, base) for exp in range(2, 10) for base in range(2, 10) if not areEq(exp, 30, base)]
#%% 
...
persone = [paola, peter, andrea,  giulia]

# 1) la lista delle persone il cui nome inizia con “P”
# 2) la lista delle coppie (nome madre, nome figlia/o)
def nome(persone, carattere):
    return [x[0] for x in persone if x[0].startswith(carattere)]

def madreFigli(persone):
    return [(x[0], y[0]) for x in persone for y in x[2] if x[1]]
#%% 
