...
def ppTree(tree, level=0):
    if isLeaf(tree): return print(tree.data, end='')
    if level>0: print("(", end='')
    ppTree(tree.sx, level+1)
    print(tree.data, end='')
    ppTree(tree.dx, level+1)
    if level>0: print(")", end='')
#%% 
...
def depthN(tree, n):
    if not tree: return []
    if n==0: return [tree.data]
    l1=depthN(tree.sx, n-1)
    l2=depthN(tree.dx, n-1)
    return l1+l2
